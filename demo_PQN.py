...
def main():
    ...
    for i in range(100):
        ...
        I[int(temp):int(0.05/dt+temp),random.randint(0,N-1)] = 0.12
    # background noise
    # for i in range(N):
    #     if i%(N//4) < N//5:
    #         I[:,i] += 1.5e-1*np.random.rand(number_of_iterations)
    #     else:
    #         I[:,i] += 1.5e-1*np.random.rand(number_of_iterations)
    ...
    for i in range(N_S):
        ...
        if r%(N//4) > N//5 or resovoir_weight[r, c] < 0:
            ...
            synapses_out1.tau_inact[i] = 0.0015
...
def show_network(resovoir_weight, N, SEED, num):
    ...
    communities_sets = nx.community.greedy_modularity_communities(G)
    pos = nx.spring_layout(G)    #   ノード配置（円形 or 自動レイアウト） 
    ...
    nx.draw_networkx_nodes(G, pos, node_size=10, node_color="lightgray")#5->10
